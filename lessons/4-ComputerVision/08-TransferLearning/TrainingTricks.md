...
Để làm cho các tính toán số học ổn định hơn, đảm bảo rằng tất cả các giá trị trong mạng nơ-ron nằm trong một phạm vi hợp lý, thường là [-1..1] hoặc [0..1]. Đây không phải là một yêu cầu quá nghiêm ngặt, nhưng bản chất của các phép tính số thực là các giá trị có độ lớn khác nhau không thể được xử lý chính xác cùng nhau. Ví dụ, nếu chúng ta cộng 10<sup>-10</sup> và 10<sup>10</sup>, kết quả có khả năng là 10<sup>10</sup>, vì giá trị nhỏ hơn sẽ bị “chuyển đổi” sang cùng bậc với giá trị lớn hơn, và do đó phần mantissa sẽ bị mất.
...
Lý tưởng nhất, các giá trị nằm trong cùng một phạm vi sau khi đi qua các lớp của mạng. Vì vậy, việc khởi tạo trọng số sao cho phân phối giá trị được bảo toàn là rất quan trọng.
...
Mặc dù nghe có vẻ kỳ lạ, bạn có thể thấy hiệu quả của dropout khi huấn luyện bộ phân loại chữ số MNIST trong notebook [`Dropout.ipynb`](https://colab.research.google.com/github/hieubqdsm/ai-for-beginner-microsoft-vi/blob/main/lessons/4-ComputerVision/08-TransferLearning/Dropout.ipynb). Nó tăng tốc độ huấn luyện và cho phép đạt được độ chính xác cao hơn trong ít epoch hơn.
...
- Nó có thể được coi là *trung bình mô hình ngầm định*, vì trong quá trình dropout, huấn luyện các mô hình hơi khác nhau
...
Một khía cạnh rất quan trọng của deep learning là khả năng ngăn chặn [overfitting](https://github.com/microsoft/AI-For-Beginners/blob/main/lessons/3-NeuralNetworks/05-Frameworks/Overfitting.md). Mặc dù có thể rất hấp dẫn khi sử dụng một mô hình mạng nơ-ron rất mạnh, luôn cần cân bằng số lượng tham số của mô hình với số lượng mẫu huấn luyện.

> Hãy đảm bảo rằng bạn hiểu khái niệm [overfitting](https://github.com/microsoft/AI-For-Beginners/blob/main/lessons/3-NeuralNetworks/05-Frameworks/Overfitting.md) mà giới thiệu trước đó!
...
Trong deep learning, **Stochastic Gradient Descent** (SGD), là gradient descent được áp dụng cho các minibatch được chọn ngẫu nhiên từ tập huấn luyện. Trọng số được điều chỉnh theo công thức:

w<sup>t+1</sup> = w<sup>t</sup> - η∇ℒ

### Momentum

Trong **momentum SGD**, giữ lại một phần gradient từ các bước trước đó. Điều này tương tự như khi chúng ta di chuyển với quán tính, và nhận một cú đẩy theo hướng khác, quỹ đạo của thay đổi ngay lập tức mà giữ lại một phần chuyển động ban đầu. Ở đây, giới thiệu một vector khác v để biểu diễn *tốc độ*:
...
Vì trong mỗi lớp, nhân tín hiệu với một ma trận W<sub>i</sub>, tùy thuộc vào ||W<sub>i</sub>||, gradient có thể hoặc giảm dần về gần 0, hoặc tăng lên vô hạn. Đây là bản chất của vấn đề Gradient Bùng Nổ/Biến Mất.
...
Gradient clipping là một mở rộng của ý tưởng trên. Khi ||∇ℒ|| ≤ θ, gradient gốc trong tối ưu hóa trọng số, và khi ||∇ℒ|| > θ - chúng ta chia gradient cho chuẩn của nó. Ở đây θ là một tham số, trong hầu hết các trường hợp lấy θ=1 hoặc θ=10.

### Giảm tốc độ học

Thành công của huấn luyện thường phụ thuộc vào tham số tốc độ học η. Có thể giả định rằng giá trị lớn của η dẫn đến huấn luyện nhanh hơn, điều mà chúng ta thường muốn ở giai đoạn đầu của huấn luyện, và sau đó giá trị nhỏ hơn của η cho phép chúng ta tinh chỉnh mạng. Do đó, trong hầu hết các trường hợp, giảm η trong quá trình huấn luyện.
