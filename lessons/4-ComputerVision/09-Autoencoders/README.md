...
Khi huấn luyện CNN, một trong những vấn đề là rất nhiều dữ liệu được gắn nhãn. Trong trường hợp phân loại hình ảnh, phân chia hình ảnh thành các lớp khác nhau, điều này đòi hỏi sự nỗ lực thủ công.

## [Pre-lecture quiz](https://ff-quizzes.netlify.app/en/ai/quiz/17)

Tuy nhiên, muốn sử dụng dữ liệu thô (không được gắn nhãn) để huấn luyện các bộ trích xuất đặc trưng của CNN, điều này được gọi là **học tự giám sát**. Thay vì sử dụng nhãn, sử dụng hình ảnh huấn luyện làm cả đầu vào và đầu ra của mạng. Ý tưởng chính của **autoencoder** là có một **mạng mã hóa** chuyển đổi hình ảnh đầu vào thành một **không gian ẩn** nào đó (thường chỉ là một vector có kích thước nhỏ hơn), sau đó là **mạng giải mã**, với mục tiêu tái tạo lại hình ảnh gốc.
...
Vì huấn luyện autoencoder để nắm bắt càng nhiều thông tin từ hình ảnh gốc càng tốt nhằm tái tạo chính xác, mạng cố gắng tìm **embedding** tốt nhất của hình ảnh đầu vào để nắm bắt ý nghĩa.
...
* **Khử nhiễu**, tức là loại bỏ nhiễu khỏi hình ảnh. Vì nhiễu mang theo rất nhiều thông tin không cần thiết, autoencoder không thể đưa tất cả vào không gian ẩn tương đối nhỏ, do đó nó chỉ nắm bắt phần quan trọng của hình ảnh. Khi huấn luyện bộ khử nhiễu, bắt đầu với hình ảnh gốc và sử dụng hình ảnh có nhiễu được thêm vào một cách nhân tạo làm đầu vào cho autoencoder.
...
* **Mô hình sinh**. Sau khi huấn luyện autoencoder, phần giải mã có thể được sử dụng để tạo ra các đối tượng mới bắt đầu từ các vector ẩn ngẫu nhiên.

## Autoencoders Biến thể (VAE)

Autoencoders truyền thống giảm chiều của dữ liệu đầu vào bằng cách nào đó, tìm ra các đặc trưng quan trọng của hình ảnh đầu vào. Tuy nhiên, các vector ẩn thường không có ý nghĩa nhiều. Nói cách khác, lấy ví dụ bộ dữ liệu MNIST, việc xác định các chữ số tương ứng với các vector ẩn khác nhau không phải là nhiệm vụ dễ dàng, vì các vector ẩn gần nhau không nhất thiết phải tương ứng với cùng một chữ số.

Mặt khác, để huấn luyện các mô hình *sinh*, tốt hơn là có một sự hiểu biết về không gian ẩn. Ý tưởng này dẫn chúng ta đến **autoencoder biến thể** (VAE).

VAE là autoencoder học cách dự đoán *phân phối thống kê* của các tham số tiềm ẩn, được gọi là **phân phối tiềm ẩn**. Ví dụ, muốn các vector ẩn được phân phối theo chuẩn với một số giá trị trung bình z<sub>mean</sub> và độ lệch chuẩn z<sub>sigma</sub> (cả giá trị trung bình và độ lệch chuẩn đều là các vector có một số chiều d). Bộ mã hóa trong VAE học cách dự đoán các tham số này, sau đó bộ giải mã lấy một vector ngẫu nhiên từ phân phối này để tái tạo đối tượng.

Tóm lại:

 * Từ vector đầu vào, dự đoán `z_mean` và `z_log_sigma` (thay vì dự đoán độ lệch chuẩn, dự đoán logarit của nó)
...
* **Mất mát KL**, đảm bảo rằng phân phối biến ẩn gần với phân phối chuẩn. Nó dựa trên khái niệm [Kullback-Leibler divergence](https://www.countbayesie.com/blog/2017/5/9/kullback-leibler-divergence-explained) - một thước đo để ước tính mức độ tương đồng giữa hai phân phối thống kê.

Một lợi thế quan trọng của VAEs là chúng cho phép chúng ta tạo ra hình ảnh mới một cách tương đối dễ dàng, vì phân phối nào để lấy mẫu các vector ẩn. Ví dụ, nếu chúng ta huấn luyện VAE với vector ẩn 2D trên MNIST, thay đổi các thành phần của vector ẩn để tạo ra các chữ số khác nhau:
...
Quan sát cách các hình ảnh hòa trộn vào nhau, khi chúng ta bắt đầu lấy các vector ẩn từ các phần khác nhau của không gian tham số tiềm ẩn. có thể trực quan hóa không gian này trong 2D:
